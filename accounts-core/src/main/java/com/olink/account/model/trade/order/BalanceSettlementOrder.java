 ...
 * B户余额结算订单
 */
@ADBTable(tablename = "acc_order_balancesettlement", isSeparate = true)
public class BalanceSettlementOrder extends OrderBase {
    ...
    public BalanceSettlementOrder() {
        super(OrderTypeEnum.BalanceSettlement);
    }
    ...
    public boolean beforeCreateOrder(ConnectionFactory connectionFactory) {
        ...
        //账务平台可用余额减少，B户可用余额减少
        this.setBeforebalance(getAccountResult().getResultBeforBalance(ChangeAccountTypeEnum.Business, AccountTypeEnum.Change, AmontTypeEnum.balance));
        this.setAfterbalance(getAccountResult().getResultAfterBalance(ChangeAccountTypeEnum.Business, AccountTypeEnum.Change, AmontTypeEnum.balance));
        return true;
    }
